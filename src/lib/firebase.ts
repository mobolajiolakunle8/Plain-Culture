...
if (isFirebaseConfigured) {
  try {
    ...
    // Safe analytics initialization for supported browser environments only.
...
export const dbService = {
  ...
  subscribeOrders: (callback: (orders: Order[]) => void) => {
    ...
    const syncOrdersToLocal = () => {
      ...
      firebaseOrdersCache.forEach((firebaseOrder) => {
        const idx = merged.findIndex((localOrder) => localOrder.id === firebaseOrder.id);
        if (idx === -1) merged.push(firebaseOrder);
        else merged[idx] = { ...merged[idx], ...firebaseOrder };
      });
