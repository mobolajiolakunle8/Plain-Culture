...
export const dbService = {
      ...
      ensureFirebaseAuthSession().then(() => {
        ...
        unsubscribeFirebase = onSnapshot(q, (snapshot) => {
          ...
          if (ordersList.length > 0) {
            const localOrders = mockDb.getData("orders");
            const merged = [...localOrders];
            ordersList.forEach(fo => {
              const idx = merged.findIndex(lo => lo.id === fo.id);
              if (idx === -1) merged.push(fo);
              else merged[idx] = { ...merged[idx], ...fo };
            });
            merged.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
            mockDb.saveData("orders", merged);
          }
        }, (error) => {
  ...
  deleteOrder: async (id: string): Promise<void> => {
    if (isFirebaseConfigured && db) {
      ensureFirebaseAuthSession()
        .then(() => deleteDoc(doc(db, "orders", id)))
