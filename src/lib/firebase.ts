...
export const dbService = {
    ...
    if (isFirebaseConfigured && db) {
      ...
      onSnapshot(q, (snapshot) => {
        ...
        if (productsList.length > 0) {
          const localProducts = mockDb.getData("products");
          const merged = [...localProducts];
          productsList.forEach(fp => {
            const idx = merged.findIndex(lp => lp.id === fp.id);
            if (idx === -1) merged.push(fp);
            else merged[idx] = { ...merged[idx], ...fp };
          });
          mockDb.saveData("products", merged);
        }
      }, (error) => {
  ...
  getProducts: async (): Promise<Product[]> => {
    if (isFirebaseConfigured && db) {
      try {
        ...
        if (productsList.length === 0) {
          const localProducts = mockDb.getData("products") as Product[];
          localProducts.forEach((product) => {
            setDoc(doc(db, "products", product.id), product).catch((e) => console.error("Firebase seed product error:", e));
          });
  ...
  subscribeOrders: (callback: (orders: Order[]) => void) => {
    const unsubscribeLocal = mockDb.subscribe("orders", callback);
    let unsubscribeFirebase: (() => void) | undefined;
    let unsubscribeDeletedOrders: (() => void) | undefined;
    let firebaseOrdersCache: Order[] = [];
    let hasFirebaseOrdersSnapshot = false;
    let deletedOrderIds = new Set<string>();

    const syncOrdersToLocal = () => {
      const localOrders = mockDb.getData("orders") as Order[];
      // Once Firestore has produced a snapshot, it becomes the source of truth.
      // This is critical for cross-browser deletes: old local cache must not re-add deleted orders.
      const sourceOrders = hasFirebaseOrdersSnapshot ? [...firebaseOrdersCache] : [...localOrders];
    ...
    if (isFirebaseConfigured && db) {
      ensureFirebaseAuthSession().then(() => {
        ...
        unsubscribeFirebase = onSnapshot(q, (snapshot) => {
          ...
          hasFirebaseOrdersSnapshot = true;
          firebaseOrdersCache = ordersList;
          syncOrdersToLocal();
        }, (error) => {
          console.warn("Firebase orders sync error:", error);
        });

        const deletedQ = query(collection(db, "deletedOrders"), orderBy("deletedAt", "desc"));
        unsubscribeDeletedOrders = onSnapshot(deletedQ, (snapshot) => {
          deletedOrderIds = new Set<string>();
          snapshot.forEach((docItem) => {
            deletedOrderIds.add(docItem.id);
          });
          syncOrdersToLocal();
        }, (error) => {
          console.warn("Firebase deleted orders sync error:", error);
        });
      });
    }
    ...
    return () => {
      ...
      if (unsubscribeDeletedOrders) unsubscribeDeletedOrders();
    };
  },
  ...
  addOrder: async (orderData: Omit<Order, "id" | "createdAt" | "status">): Promise<Order> => {
    ...
    // 1. Save order asynchronously so offline/slow Firebase connection never blocks checkout
    if (isFirebaseConfigured && db) {
      ensureFirebaseAuthSession()
        .then(() => setDoc(doc(db, "orders", newOrder.id), newOrder))
        .then(() => console.log("Order saved to Firebase!"))
        .catch((e) => console.error("Firebase addOrder error:", e));
    }
  ...
  updateOrderStatus: async (id: string, status: "Pending" | "Confirmed" | "Delivered"): Promise<void> => {
    ...
    if (isFirebaseConfigured && db) {
      ensureFirebaseAuthSession()
        .then(() => updateDoc(doc(db, "orders", id), { status }))
        .then(() => console.log("Order status updated in Firebase!"))
        .catch((e) => console.error("Firebase updateOrderStatus error:", e));
    }
  },
    ...
    if (isFirebaseConfigured && db) {
      ensureFirebaseAuthSession()
        .then(async () => {
          ...
          await setDoc(doc(db, "deletedOrders", id), {
            id,
            deletedAt: new Date().toISOString()
          });
        })
        .catch((e) => console.error("Firebase deleteOrder error:", e));
    }
  },
