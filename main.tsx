...
export const CartProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const submitCheckout = async (checkoutForm: { name: string; phone: string; address: string; email?: string; deliveryFee?: number; deliveryLocation?: string; proofFile?: File | null }) => {
    ...
    try {
      ...
      // 3. Upload payment proof if provided
      let paymentProofUrl = "";
      if (checkoutForm.proofFile) {
        try {
          paymentProofUrl = await dbService.uploadPaymentProof(checkoutForm.proofFile, "temp");
        } catch (uploadError) {
          console.warn("Payment proof upload failed, proceeding without proof:", uploadError);
        }
      }
